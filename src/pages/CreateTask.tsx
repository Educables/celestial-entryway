...
export default function CreateTask() {
  ...
  const courseMaterials = searchParams.get('materials')?.split(',').filter(Boolean) || [];
      ...
      <div className="border-b">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold">Create Homework Task</h1>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-4">
            <Label className="text-lg font-semibold">Questions and Options</Label>
            <div className="grid gap-6">
              {Array.from({ length: numQuestions }, (_, i) => i + 1).map((qNum) => {
                const options = questionOptions[qNum] || [];
                return (
                  <div key={qNum} className="border rounded-lg p-6 bg-card">
                    <Label className="text-base font-semibold mb-4 block">Question {qNum}</Label>
